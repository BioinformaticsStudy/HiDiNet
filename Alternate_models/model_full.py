...
class Model(nn.Module):
    def __init__(self, device, N, gamma_size, z_size, decoder_size, Nflows, flow_hidden, mean_T, std_T, dt = 0.5, length = 25):
        ...
        self.memory0 = Memory(z_size, 2*(total-N)-2, self.gamma_size).to(device)
        self.dynamics = SDEModel(z_size, device, 2*(total-N)-2, self.gamma_size, mean_T, std_T).to(device)
