...
def build_data_start(full_data, full_times, full_survival, full_env, full_med, deficit_names, background_names, medication_names, M, N, t_length, start, min_count, prune = True):
    ...
    for i,id in enumerate(full_data['id'].unique()):
        ...
        indiv = full_data.loc[full_data['id'] == id, deficit_names]
        indiv_times = full_times.loc[full_times['id'] == id, 'age']
        indiv_S = full_survival.loc[full_survival['id'] == id, 'death age']
        indiv_env = full_env.loc[full_env['id'] == id, background_names]
        indiv_med = full_med.loc[full_med['id'] == id, medication_names]
...
class Dataset(data.Dataset):
    ...
    def __init__(self, name, N, pop = False, prune = True, min_count = 1):
        ...
        if os.path.isdir(folder) and not pop:
            ...
            for i in range(-1, 10):
                data_, times_, death_age_, mask_, survival_mask_, censored_, env_, med_time_, dead_mask_, after_dead_mask_, ids_ = build_data_start(X, T, A, E, Med, deficits, background, medications, num_indiv, N, t_length, i, min_count, prune)
                data.append(data_)
                times.append(times_)
