...
for N in Ns:
    ...
    G = [G[int(np.nanmin(survival_ages[i])):][:len(survival_ages[i])] for i in range(len(censored))]
    ...
    IBS = np.trapezoid(y = BS_t[ (bin_centers>=min_death_age) & (bin_centers<=max_death_age) ], x = bin_centers[ (bin_centers>=min_death_age) & (bin_centers<=max_death_age) ])/(max_death_age-min_death_age)
    results['IBS'][N] = IBS
...
if lamp_compare:
    with open(f'{dir}/../Analysis_Data_elsa/IBS_job_id{args.lamp_job_id}_epoch{args.lamp_epoch}.txt','r') as infile:
        value = infile.readlines()
        lamp_IBS = float(value[0])
...
if djin_compare and lamp_compare:
    custom_legend = []
    labels = []
    plt.scatter(29, lamp_IBS, color='b', marker='v', zorder=5)
    plt.scatter(x=29,y=djin_IBS,color='orange')
...
fig.savefig(f'{dir}/../Plots/latent_brier_score_by_dim_job_id{args.lamp_job_id}_epoch{args.lamp_epoch}{postfix}.pdf')
